...
- **`/implement ABC-123`** — fetches Jira + Zephyr + Figma, reads sibling-ticket memory, writes tests in the project's own style, then saves a per-ticket memory file.
...
│                                #   └─ ## Agent Settings: agent_run toggle
...
By default the agent writes code and tells you the test command to run yourself.  
To let the agent run tests, self-correct on failure (up to 3 retries), and report results automatically, open [.github/PROJECT_CONTEXT.md](.github/PROJECT_CONTEXT.md) and change:

```md
## Agent Settings
- `agent_run: false`   ← change to true
```

No restart needed — the orchestrator reads this on every task.
